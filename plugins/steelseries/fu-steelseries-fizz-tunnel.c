...
fu_steelseries_fizz_tunnel_probe(FuDevice *device, GError **error)
{
	...
	fu_device_add_instance_str(device, "STEELSERIES", "FIZZ");
	...
	fu_device_build_instance_id_quirk(device, NULL, "USB", "STEELSERIES", "VID", NULL);
	fu_device_build_instance_id(device, NULL, "USB", "STEELSERIES", "VID", "PID", NULL);
	fu_device_build_instance_id(device, NULL, "USB", "STEELSERIES", "VID", "PID", "REV", NULL);
...
fu_steelseries_fizz_tunnel_poll(FuDevice *device, GError **error)
{
	...
	if (!fu_steelseries_fizz_file_crc32(parent,
					    ...
					    &stored_crc,
					    &error_local)) {
		g_debug("ignoring error on get file CRC32 from FS 0x%02x ID 0x%02x: %s", fs, id, error_local->message);
...
fu_steelseries_fizz_tunnel_read_firmware(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	buf = g_malloc0(bufsz);
	if (!fu_steelseries_fizz_read_access_file(parent,
						  TRUE,
						  fs,
						  id,
						  buf,
...
static FuFirmware *
fu_steelseries_fizz_tunnel_prepare_firmware(FuDevice *device,
					    GBytes *fw,
					    FwupdInstallFlags flags,
					    GError **error)
{
...
fu_steelseries_fizz_tunnel_class_init(FuSteelseriesFizzTunnelClass *klass)
{
	...
	klass_device->prepare_firmware = fu_steelseries_fizz_tunnel_prepare_firmware;
...
fu_steelseries_fizz_tunnel_init(FuSteelseriesFizzTunnel *self)
{
	...
	fu_device_add_protocol(FU_DEVICE(self), "com.steelseries.fizz.tunnel");
	...
	fu_device_set_poll_interval(FU_DEVICE(self), 10000); /* 1 min */
